...
class VacuumPlanning(Problem):
    ...
    def path_cost(self, curNode, state1, action, state2):
        ...
        cost = curNode.path_cost  # Start with the accumulated cost from the current node

        if self.env.costFunc == costFunctions[0]:  
            # Basic step count cost: Each move has a fixed cost of 1
            cost += 1  

        elif self.env.costFunc == costFunctions[1]:  
            # Step count + turn cost: Adds a penalty for changing direction
            turn_cost = self.computeTurnCost(curNode.action, action)
            cost += 1 + turn_cost  

        elif self.env.costFunc == costFunctions[2]:  
            # StayLeft: Encourages cleaning the left half of the grid before the right half
            cost += 1  # Default step cost
            if state2[0] < state1[0]:  # Moving left
                cost -= 0.5  # Reducing cost makes left-side movement preferable

        else:  
            # StayUp: Encourages cleaning the top half of the grid before the bottom half
            cost += 1  # Default step cost
            if state2[1] < state1[1]:  # Moving downward
                cost -= 0.5  # Reducing cost makes upward movement preferable
    ...
    def computeTurnCost(self, action1, action):
        ...
        if action1 is None or action is None:
            return 0  # No cost if there’s no direction change

        if action1 == action:
            return 0  # No cost for moving in the same direction

        case = action1 + action # Combine current and new direction
        turncost = 0

        # Check for a 90-degree turn (cost = 3)
        if case in [
            "UPRIGHT", "UPLEFT",      # Up → Right/Left
            "DOWNRIGHT", "DOWNLEFT",  # Down → Right/Left
            "LEFTUP", "LEFTDOWN",     # Left → Up/Down
            "RIGHTUP", "RIGHTDOWN"    # Right → Up/Down
        ]:
            turncost += 3
        # Check for a 180-degree turn (cost = 6)
        elif case in[
            "UPDOWN", "DOWNUP",
            "LEFTRIGH", "RIGHTLEFT"
        ]:
        ...
        return turncost # Return the computed turn cost
    ...
    def findMinManhattanDist(self, pos):
        ...
        min_dist = sys.float_info.max  # Initialize to the maximum possible float value

        for single_thing in self.env.things:
            if isinstance(single_thing, Dirt):  # Check if it's a dirty room
                dist = distance_manhattan(pos, single_thing.location)  # Use the provided Manhattan distance function
                if dist < min_dist:
                    min_dist = dist

        if min_dist != sys.float_info.max:
            return min_dist  # Return the minimum distance
        else:
            return 0  # No dirty rooms, return 0

    def findMinEuclidDist(self, pos):
        """Find a dirty room among all dirty rooms which has minimum Manhattan distance to pos
                hint: use distance_manhattan() function in utils.py"""
        min_dist = sys.float_info.max  # Set to max float initially

        for single_thing in self.env.things:
            if isinstance(single_thing, Dirt):
                dist = distance_euclid(single_thing.location, pos) ** 2  # Compute squared Euclidean distance
                if dist < min_dist:
                    min_dist = dist  # Update minimum distance
                
        if min_dist != sys.float_info.max: # Return min distance or 0 if no dirt found
            return min_dist
        else:
            return 0
...
def breadth_first_graph_search(problem):
    ...
    """
    node = Node(problem.initial)  # Initialize with the starting node

    if problem.goal_test(node.state):  # If initial state is the goal, return it
        return node, None

    frontier = deque([node])  # FIFO queue for BFS
    visited = set()  # Track visited states

    while frontier:  # Loop while nodes remain in frontier
        node = frontier.popleft()  # Get next node from queue
        visited.add(tuple(node.state))  # Mark state as visited

        if problem.goal_test(node.state):  # If goal is reached, return solution
            return node, visited

        for child in node.expand(problem):  # Generate child nodes
            c_state = tuple(child.state)

            if problem.goal_test(c_state):  # If child state is goal, return it
                return child, visited

            if c_state not in visited:  # If child state is unvisited, add to queue
...
def depth_first_graph_search(problem):
    ...
    node = Node(problem.initial)  # Start with the initial node
    frontier = deque([node])  # Stack (LIFO)
    explored = set()  # Track visited states

    if problem.goal_test(node.state):  # If initial state is goal, return it
        return node, None

    while frontier:
        node = frontier.pop()  # Get last node from stack
        state_tuple = tuple(node.state)  # Convert to tuple for tracking
        explored.add(state_tuple)

        if problem.goal_test(state_tuple):  # Check if goal reached
        ...
        for child in node.expand(problem):
            ...
            if child_state_tuple not in explored and child not in frontier:
                if problem.goal_test(child.state):  # If child is goal, return it
                    return child, explored
                frontier.append(child)  # Otherwise, add child to stack
...
def best_first_graph_search(problem, f=None):
    ...
    node = Node(problem.initial)  # Start with initial node
    f = memoize(f or problem.h, 'f')  # Use heuristic function if not provided
    frontier = PriorityQueue('min', f)  # Priority queue based on f-value
    fronStateToNode = {}  # Map state to node
    visited = set()  # Track visited states

    frontier.append(node)  # Add start node to queue

    while frontier:
        node = frontier.pop()  # Get lowest-cost node
        
        if problem.goal_test(node.state):  # If goal found, return it
            return node, visited

        visited.add(tuple(node.state))  # Mark node as visited

        for child in node.expand(problem):
            if tuple(child.state) not in visited and child not in frontier:
                frontier.append(child)
                fronStateToNode[tuple(child.state)] = child  # Update state mapping

            elif child in frontier:
                current = fronStateToNode[tuple(child.state)]  # Get current node in queue
                if f(child) < f(current):  # If new path is better, replace it
                    del fronStateToNode[tuple(current.state)]
                    del frontier[current]
                    frontier.append(child)
                    fronStateToNode[tuple(child.state)] = child
